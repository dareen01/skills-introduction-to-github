 ...
 * 
 * FIXED: Changed RGB check from == 0 (black) to == 0 (actually checking alpha+RGB)
 * The reference code checks if pixel is BLACK (0), not white.
 * This affects brightness calculations in SubImgCharMatcher.
 */
public class CharConverter {
     ...
     * @return 2D boolean array where true = black pixel (character drawn), false = white (background)
     */
    public static boolean[][] convertToBoolArray(char c) {
        ...
        for(int y = 0 ; y < DEFAULT_PIXEL_RESOLUTION ; y++) {
            for(int x = 0 ; x < DEFAULT_PIXEL_RESOLUTION ; x++) {
                // FIXED: Check for black (0) not white - this is the correct interpretation
                // true = pixel is part of the character (black/drawn)
                // false = pixel is background (white/empty)
    ...
    /**
     * Creates a BufferedImage with the character rendered in the specified font.
     * 
     * @param c the character to render
     * @param fontName the font name to use
     * @param pixelsPerRow the size of the image in pixels
